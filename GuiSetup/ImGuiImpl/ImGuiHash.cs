...
internal static class ImGuiHash
{
    private static ReadOnlySpan<uint> GCrc32LookupTable =>
    [
    ...
    public unsafe static uint ImHashData(ReadOnlySpan<byte> span, uint seed = 0)
    {
        fixed (byte* ptr = span)
        {
            return ImHashData(ptr, (nuint)span.Length, seed);
        }
    }
    ...
    public unsafe static uint ImHashData(void* data, nuint dataSize, uint seed = 0)
    {
        ...
        while (dataSize-- != 0)
        {
            crc = (crc >> 8) ^ crc32_lut[(int)((crc & 0xFF) ^ *bytes++)];
        }
